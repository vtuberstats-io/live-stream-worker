...
async function fetchNextContinuation(videoId, playerOffsetMs, continuation) {
  const url = `https://www.youtube.com/live_chat_replay/get_live_chat_replay?pbj=1&hidden=false&continuation=${continuation}`;
...
async function handleContinuation(continuations) {
  const { continuation, timeUntilLastMessageMsec } = continuations.find((item) =>
    Object.keys(item).includes('liveChatReplayContinuationData')
  ).liveChatReplayContinuationData;
  return [continuation, timeUntilLastMessageMsec];
}
...
async function* fetchLiveChatMessages(videoId, onNewContinuation, lastContinuation) {
  let continuation = lastContinuation || (await fetchInitialContinuation(videoId));
  while (continuation) {
    ...
    const [newContinuation, timeUntilLastMessageMsec] = await handleContinuation(
      liveChatContinuation.continuations
    );

    await onNewContinuation(newContinuation);

    await wait(timeUntilLastMessageMsec);
